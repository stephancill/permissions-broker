...
- Added read-only email access over generic IMAP (`/v1/email`): a broker-hosted connect form (`/v1/accounts/connect/imap`) auto-detects the IMAP server from the email domain (Thunderbird ISPDB autoconfig + domain heuristics, verified by a real login), stores the encrypted credential in `linked_accounts` (`provider='imap'`), and exposes three curated read-only operations (list folders, search, read by UID) gated by short-lived, key-scoped, Telegram-approved sessions (`email_sessions`, `es:` callbacks).
- Read-only is enforced by construction: the IMAP client (`src/email/client.ts`) is the only socket owner, `ops.ts` exposes only `list`/`search`/`fetch`/`fetchOne`, and every mailbox is opened with IMAP `EXAMINE` (`getMailboxLock(path, { readOnly: true })`). The client depends on `imapflow` + `mailparser`, which run on the Bun runtime and natively on Cloudflare Workers via `node:net`/`node:tls` (TCP Sockets API); sessions are swept by `src/email/sweeper.ts`.
