...
- Added read-only email access over generic IMAP (`/v1/email`): a broker-hosted connect form (`/v1/accounts/connect/imap`) auto-detects the IMAP server from the email domain (Thunderbird ISPDB autoconfig + MX-based provider detection + domain heuristics, verified by a real login), stores the encrypted credential in `linked_accounts` (`provider='imap'`), and exposes three curated read-only operations (list folders, search, read by UID) gated by short-lived, key-scoped, Telegram-approved sessions (`email_sessions`, `es:` callbacks).
- Read-only is enforced by construction: the IMAP client (`src/email/protocol.ts` + `src/email/client.ts`) implements only LIST/EXAMINE/SEARCH/read FETCHs, and every mailbox is opened read-only via IMAP `EXAMINE`. It uses a hand-rolled protocol client on `node:net`/`node:tls` (with event-loop yields for large literals) rather than `imapflow`, because `imapflow`'s stream handling does not work on the Cloudflare Workers `node:net`/`node:tls` shim while raw sockets do. `mailparser` handles MIME decoding. Sessions are swept by `src/email/sweeper.ts`.
